...
func TestParse_InvalidJSON(t *testing.T) {
	...
	_ = os.WriteFile(f, []byte("{invalid json"), 0o644)
...
func TestParse_MissingOpenapiField(t *testing.T) {
	...
	_ = os.WriteFile(f, []byte(`{"info": {"title": "Test", "version": "1.0"}}`), 0o644)
	_, err := parser.Parse(f, false)
	if err == nil {
...
func TestParse_UnsupportedVersion(t *testing.T) {
	...
	_ = os.WriteFile(f, []byte(`{"openapi": "2.0", "info": {"title": "T", "version": "1"}}`), 0o644)
	_, err := parser.Parse(f, false)
	if err == nil {
